...
@section('content')
    <marquee><h2><font color='blue'>新闻 修改</font></h2></marquee>
    <form class="layui-form layui-form-pane" action="{{ url("/admin/new_update") }}" method="post" style="margin: 50px ;" id="form" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="new_id" value="{{ $new->new_id }}">
        <div class="layui-form-item" style="padding-left:300px">
            <label class="layui-form-label">标题</label>
            <div class="layui-input-inline">
                <input type="text" name="title" value="{{ $new->title }}"  required  lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input">@php echo $errors->first('brand_name')@endphp
            </div>
        </div>
        ...
        <div class="layui-form-item" style="padding-left:300px">
            ...
            <div class="layui-input-block" >
                <input type="radio" name="is_hot" value="1" title="是" @if($new->is_hot == 1)checked @endif>
                <input type="radio" name="is_hot" value="0" title="否" @if($new->is_hot == 0)checked @endif>
            </div>
        </div>
        ...
        <div class="layui-form-item" style="padding-left:300px">
            ...
            <div class="layui-input-block" >
                <input type="radio" name="new_show" value="1" title="是" @if($new->new_show == 1)checked @endif>
                <input type="radio" name="new_show" value="0" title="否" @if($new->new_show == 0)checked @endif>
            </div>
        </div>
                ...
                <select name="cid" lay-verify="required" lay-search>
                    @foreach($cate as $k=>$v)
                        <optgroup label="{{ $v->nav_name }}">
                            <option value="{{ $v->cid }}" @if($v->cid == $new->cid) selected @endif>{{ $v->cname }}</option>
                        </optgroup>
                    @endforeach
                </select>
            </div>
